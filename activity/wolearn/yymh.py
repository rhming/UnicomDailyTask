...
class YYMHAct(WoLearn):

    def __init__(self, mobile, password):
        super(YYMHAct, self).__init__(mobile, password)
        self.chc = "BCEBcVJlVy4BeVYiBjxbZFF4UD0"
    ...
    def getReward(self, item):
        url = 'https://edu.10155.com/wxx-api/Api/YYMHAct/getReward'
        data = {
            "p": "",
    ...
    def raffle(self):
        url = 'https://edu.10155.com/wxx-api/Api/YYMHAct/raffle'
        data = {
            "p": "",
            "chc": self.config.get('chc'),
            "jrPlatform": "ACTIVITY",
        ...
        result = resp.json()
        try:
            reward_name = f"盲盒_{self.now_time}_{result['data']['ybl_reward_name']}"
    ...
    def userActInfo(self, debug=False):
        url = 'https://edu.10155.com/wxx-api/Api/YYMHAct/userActInfo'
        data = {
            "p": "",
            "chc": self.config.get('chc'),
            "jrPlatform": "ACTIVITY",
    ...
    def addRaffleChance(self, orderId, tip):
        url = 'https://edu.10155.com/wxx-api/Api/YYMHAct/addRaffleChance'
        data = {
            "p": "",
            "chc": self.config.get('chc'),
            "jrPlatform": "ACTIVITY",
    ...
    def handlePrize(self):
        for item in self.prizeList:
            if int(item['ybl_reward_status']) or int(item['ybl_reward_id']) in [3, 6, 11, 12]:
                continue
            else:
                print(item)
                self.getReward(item)
    ...
    def run(self):
        ...
        if not self.isLogin or self.config['timestamp'][:8] != self.now_date.replace('-', ''):
            ...
            self.openPlatLineNew(
                'https://edu.10155.com/wact/yymh.html?jrPlatform=SHOUTING&chc=BCEBcVJlVy4BeVYiBjxbZFF4UD0&vid=-1'
            )
        ...
        if lottery_times == 0 or lottery_times == lottery_chance:
            ...
            options = {
                ...
                'arguments2': '',
                'codeId': 946878414,
                'channelName': 'android-短视频激励广告活动2-激励视频',
                'remark': '短视频激励广告活动2-激励视频',
